#### NOTICE:
This is a fork of https://github.com/xlladdins/xll. The main difference is that the build system is now based on CMake
rather than Visual Studio .sln files, and the folder structure has been cleaned up a bit. The code itself is identical.
The purpose of using CMake is that it enables me to use @Jetbrains CLion. Note that Visual Studio still needs to 
be installed, as Excel relies on the MSVC ABI.
